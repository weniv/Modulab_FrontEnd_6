...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    # 2. GET 요청이 들어왔어? 내가 처리해줄게!
    def do_GET(self):  # GET 요청 처리
        parsed_url = urlparse(self.path)  # URL 파싱
        query_params = parse_qs(parsed_url.query)  # 쿼리 파싱

        # 3번 GET으로 들어왔어? 어떤값이 들어왔는지 볼까?
        print("Path:", parsed_url.path)
        print("Query Parameters:", query_params)

        # 4. username과 password 값을 가져옴. 값이 없는 경우 None이 기본값
        username = query_params.get("username", [None])[0]
        password = query_params.get("password", [None])[0]

        # 5. username과 password가 모두 존재하는지 확인해서 응답을 어떻게 할지 결정하는 로직
